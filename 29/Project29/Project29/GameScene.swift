...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func createBuildings() {
        ...
        while currentX < 1024 {
            let size = CGSize(
                width: RandomInt(min: 2, max: 4) * 40,
    ...
    func launch(angle angle: Int, velocity: Int) {
        // calculate the speed at which to launch the banana
        let speed = Double(velocity) / 10.0

        // calculate the angle at which to launch the banana
        let radians = deg2rad(angle)

        // ensure that the previous banana is gone
        if banana != nil {
        ...
        if let firstNode = firstBody.node {
            if let secondNode = secondBody.node {
                if firstNode.name == "banana" && secondNode.name == "building" {
                    guard let building = secondNode as? BuildingNode else { return }
                    bananaHitBuilding(building, atPoint: contact.contactPoint)
                }
    ...
    override func update(currentTime: CFTimeInterval) {
        if banana != nil && banana.position.y < -1000 {
            banana.removeFromParent()
            banana = nil
            changePlayer()
        }
